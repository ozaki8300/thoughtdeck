...
function buildContextId(relativePath?: string) {
  if (!relativePath) {
    return "";
  }
...
function deckLifecycleKey(deck: Deck) {
  return normalizeDeckPath(deck.relativePath) || deckKey(deck);
}

function synchronizeDecks(
  importedDecks: Deck[],
  existingDecks: Deck[],
) {
  const importedMap = new Map(
    importedDecks.map((deck) => [
      deckLifecycleKey(deck),
      deck,
    ]),
  );
  const syncedKeys = new Set<string>();
  const syncedDecks = existingDecks
    .filter((deck) =>
      importedMap.has(deckLifecycleKey(deck)),
    )
    .map((deck) => {
      const key = deckLifecycleKey(deck);
      syncedKeys.add(key);

      return importedMap.get(key) ?? deck;
    });

  for (const importedDeck of importedDecks) {
    const key = deckLifecycleKey(importedDeck);

    if (syncedKeys.has(key)) continue;

    syncedDecks.push(importedDeck);
    syncedKeys.add(key);
  }
...
export default function MyDecksPage() {
  ...
  const pickVault = async () => {
    if (!canUseDirectoryPicker()) {
      setImportMessage(
        "この端末ではVault選択は利用できません。Markdownの取り込みを使ってください。",
      );
      return;
    }
  ...
  const mergeImportedDecks = (
    ...
    if (loaded.length === 0) {
      if (synchronize) {
        if (stats) {
          stats.updated = 0;
          stats.added = 0;
          stats.removed = decks.length;
        }
        setDecks([]);
        setHighlightedKeys([]);
        setImportMessage(
          successMessage ??
            `0件追加 / 0件更新 / ${decks.length}件削除`,
        );
    ...
    const loadedKeys = loaded.map((deck) =>
      synchronize
        ? deckLifecycleKey(deck)
        : deckKey(deck),
    );
    ...
    const removedCount =
      synchronize
        ? decks.filter((deck) =>
            !loadedKeys.includes(deckLifecycleKey(deck)),
          ).length
        : 0;
    ...
    setDecks((current) => {
      ...
      for (const imported of loaded) {
        ...
        if (index >= 0) {
          const currentDeck = next[index];

          const currentTime = new Date(
            currentDeck.updated_at ||
              currentDeck.created_at,
          ).getTime();

          const importedTime = new Date(
            imported.updated_at ||
              imported.created_at,
          ).getTime();
    ...
    setImportMessage(
      successMessage ??
        `${addedCount}件追加 / ${updatedCount}件更新${
          stats?.removed ? ` / ${stats.removed}件削除` : ""
        }`,
    );
  ...
  const scanVault = async () => {
    if (!canUseDirectoryPicker()) {
      setImportMessage(
        "この端末ではVaultスキャンは利用できません。Markdownの取り込みを使ってください。",
      );
      return;
    }
  ...
  const handleFiles = async (files: File[]) => {
    ...
    const loaded = (
      await Promise.all(
        files.map(async (file): Promise<Deck | null> => {
          const parsed = parseDeck(await file.text());
  ...
  const openDeck = (
    ...
    params.set("currentTitle", deck.title);

    const relativePath = deck.relativePath ?? "";
    const lastSeparatorIndex = relativePath.lastIndexOf("/");
    ...
    const contextId =
      deck.contextId || buildContextId(deck.relativePath);
    const siblingTitles = contextId
      ? Array.from(
          new Set(
            decks
              .filter((item) =>
                (item.contextId || buildContextId(item.relativePath)) ===
                contextId,
              )
              .sort((a, b) => {
            ...
            <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="grid gap-2 sm:hidden">
                <button
                  ...
                  className="w-full rounded-lg border border-[var(--td-accent-border)] px-4 py-2 text-sm text-[var(--td-accent)] transition hover:bg-[var(--td-hover)]"
                >
                  取り込む
                </button>
                <p className="text-xs text-[var(--td-muted)]">
                  スマホではMarkdownファイルを選択して取り込んでください
                </p>
              </div>
              ...
              {importMessage && (
                <div className="absolute right-0 top-full z-20 mt-2 max-w-[min(28rem,calc(100vw-2.5rem))] rounded-lg border border-[var(--td-border)] bg-[var(--td-panel)] px-3 py-2 text-[11px] text-[var(--td-muted)] opacity-90 shadow-lg backdrop-blur">
                  {importMessage}
                </div>
              )}
                  ...
                  {group.groupKey !== "Ungrouped" && (
                    <div className="mb-1 sm:mb-0">
                      ...
                      {formatRelativeTime(group.latestUpdatedAt) && (
                        <p className="mt-0.5 text-xs text-[var(--td-muted)] opacity-45">
                          {formatRelativeTime(group.latestUpdatedAt)}
                        </p>
                      )}
                    </div>
                      ...
                      >
                        ...
                        {!isLineageExpanded && previewDeck && (
                          <article
                            ...
                            className={`group relative min-w-0 cursor-pointer overflow-hidden rounded-2xl border border-[var(--td-border)] bg-[var(--td-panel)] px-4 py-3 shadow-sm transition-all duration-200 hover:border-[var(--td-border-strong)] hover:shadow-sm ${
                                  ...
                                  {[1, 2, 3, 4, 5].map((value) => (
                                    <button
                                      ...
                                      className={`text-sm transition ${
                                        value <= previewDeck.star
                                          ? "text-yellow-300 opacity-90 hover:text-yellow-200"
                                          : "text-yellow-100/30 hover:text-yellow-200/70"
                                      }`}
                                      title={`${value} stars`}
                                    >
                                      ★
